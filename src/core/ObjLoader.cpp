...
#define IS_NEW_LINE( x ) ( ( (x) == '\r') || ( (x) == '\n') || ( (x) == '\0') )
    ...
    std::vector<std::shared_ptr<Shape>> ObjLoader(std::string fullpath, const Transform* trans, const Transform* invTrans)
    {
		...
		std::vector<material_t> materials;
		std::string err;
		if (!LoadObj(shapes, materials, err, fullpath.c_str(), nullptr,
			/*ptexQuads ? 0 : */load_flags_t(triangulation))) 
        {
			std::cout << "[ERROR] loading OBJ file: " << fullpath << ", err msg: "
				<< err << std::endl;
		...
		for (const shape_t& shape : shapes) {
			...
			std::string meshName = "MainMesh";

			//int nTriangles = vertices.size() / 3;
			//int nVertices = vertices.size();

            Transform o2w, w2o(Inverse(o2w));
			...
			std::vector<std::shared_ptr<Shape>> tShape = CreateTriangleMesh(&o2w, &w2o,
				false, mesh.num_vertices.size(), &indices[0], P.size(),
				&P[0], nullptr, &N[0], st.size() > 0 ? &st[0] : nullptr, nullptr, nullptr);
		}
