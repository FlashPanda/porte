...
// “标准的”分布
void StratifiedSample2DCanonical(Point2f* samp, int nx, int ny, RNG& rng, bool jitter)
{
	Float dx = (Float)1 / nx, dy = (Float)1 / ny;
	for (int y = 0; y < ny; ++y)
		for (int x = 0; x < nx; ++x) {
			Float jx = jitter ? rng.UniformFloat() : 0.5f;
			Float jy = jitter ? rng.UniformFloat() : 0.5f;
			samp->x = std::min((x + (y + jx) * dy) * dx, OneMinusEpsilon);
			samp->y = std::min((y + (x + jy) * dx) * dy, OneMinusEpsilon);
			++samp;
		}
}
