...
struct LinearBVHNode {
    Bounds3f bounds;
    union {
        int primitivesOffset;   // 叶子
        int secondChildOffset;  // 内部
    };
    uint16_t nPrimitives;  // 0 -> 内部节点
    uint8_t axis;          // 内部节点: xyz
    uint8_t pad[1];        // 确保32字节
};
...
inline uint32_t LeftShift3(uint32_t x) {
    //CHECK_LE(x, (1 << 10));
    if (x == (1 << 10)) --x;
#ifdef PORTE_HAVE_BINARY_CONSTANTS
...
std::shared_ptr<BVHAccel> CreateBVHAccelerator(
    ...
    std::string splitMethodName = "sah";
